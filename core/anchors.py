...
def hard_nms(cdds, topn=10, iou_thresh=0.25):
    if not (
        type(cdds).__module__ == "numpy" and len(cdds.shape) == 2 and cdds.shape[1] >= 5
    ):
...
if __name__ == "__main__":
    a = hard_nms(
        np.array([[0.4, 1, 10, 12, 20], [0.5, 1, 11, 11, 20], [0.55, 20, 30, 40, 50]]),
        topn=100,
        iou_thresh=0.4,
    )
    print(a)
